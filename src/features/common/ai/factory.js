...
const PROVIDERS = {
  ...
  'openai-clueless': {
      name: 'OpenAI (Clueless)',
      handler: () => require("./providers/openai"),
      llmModels: [
          { id: 'gpt-4.1-clueless', name: 'GPT-4.1 (clueless)' },
      ],
      sttModels: [
          { id: 'gpt-4o-mini-transcribe-clueless', name: 'GPT-4o Mini Transcribe (clueless)' }
      ],
  },
  'gemini': {
      name: 'Gemini',
...
function sanitizeModelId(model) {
  return (typeof model === 'string') ? model.replace(/-clueless$/, '') : model;
}

function createSTT(provider, opts) {
  if (provider === 'openai-clueless') provider = 'openai';
...
function getProviderClass(providerId) {
    ...
    // Handle special cases for clueless providers
    let actualProviderId = providerId;
    if (providerId === 'openai-clueless') {
